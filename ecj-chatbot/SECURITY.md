...
least 1.5.9 and is currently unpatched upstream.
...
(`data/index/`) and never binds a network socket. `diagnose.py` uses
`EphemeralClient`, which is likewise in-process. `HttpClient`,
`chromadb.Server`, and the `chroma run` CLI are not used anywhere in this
repository.

Note that `requirements.txt` pins `chromadb>=1.5.9`. That floor is **not** a
fix for this CVE — no fixed release exists yet. It is there because the
alternative, capping below 1.0.0, pulls an old `starlette` carrying nine
advisories of its own, trading a non-reachable issue for reachable ones.
